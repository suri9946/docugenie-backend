...
const validateGenerateRequest = (req, res, next) => {
  const { title, rawText, referenceText, instructions, subject, style, wordCount, customWordCount, generationMode } = req.body || {};
  const errors = [];
  ...
  const normalizedMode = typeof generationMode === "string" ? generationMode.trim().toLowerCase() : "draft";
  const rawTextRequired = normalizedMode === "draft";
  if ((rawTextRequired && (typeof rawText !== "string" || rawText.trim().length === 0)) || (!rawTextRequired && rawText !== undefined && rawText !== null && typeof rawText !== "string")) {
  ...
  if (wordCount !== undefined && wordCount !== null && Number.isNaN(Number(wordCount))) {
    errors.push("wordCount must be numeric when provided.");
  }
  if (customWordCount !== undefined && customWordCount !== null && Number.isNaN(Number(customWordCount))) {
    errors.push("customWordCount must be numeric when provided.");
  }
  ...
  req.body = {
    ...
    wordCount: wordCount === undefined || wordCount === null ? undefined : Number(wordCount),
    customWordCount: customWordCount === undefined || customWordCount === null ? undefined : Number(customWordCount),
