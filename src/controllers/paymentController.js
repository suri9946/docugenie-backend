...
const handleRazorpayWebhook = async (req, res, next) => {
  try {
    ...
    if (!webhookSecret) {
      return res.status(503).json(errorResponse("Webhook secret is not configured."));
    }
    ...
    if (!signature || signature !== expectedSignature) {
      return res.status(403).json(errorResponse("Invalid webhook signature"));
    }

    const event = typeof req.body === "object" ? req.body : JSON.parse(payload);
    const paymentEntity = event?.payload?.payment?.entity;
    const documentId =
      paymentEntity?.notes?.documentId ||
      event?.payload?.order?.entity?.notes?.documentId;

    if (event?.event === "payment.captured" && documentId) {
      await markDocumentAsPaid(documentId);
      logger.info("Razorpay webhook captured payment", {
        documentId,
